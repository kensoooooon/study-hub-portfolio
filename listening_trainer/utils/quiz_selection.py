...
from listening_trainer.models import StudentListeningPassageProgress
...
def get_pruned_progresses(student, source_type, max_count=100):
    ...
    """
    progresses = StudentListeningPassageProgress.objects.filter(
        student=student,
...
def select_passages_for_student(student, source_type="textbook", top_k=10, temperature=1.0, max_progresses=100):
    ...
    Returns:
        List[StudentListeningPassageProgress]: Softmax に基づいて選ばれた出題対象の進捗オブジェクト群

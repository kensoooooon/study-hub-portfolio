...
作成経緯:
    - read_trainer/tests/test_quiz_type_select.py を listening_trainer 向けに横展開
    - 管理者,生徒と通常,英検のいずれのクイズ選択にもテストがないことが指摘
...
class AdminBothQuizTypeSelectTest(TestCase):
    ...
    @patch("listening_trainer.views.admin_views.has_vocab_progress", return_value=True)
    @patch("listening_trainer.views.admin_views.select_passages_for_student")
    ...
    def test_admin_access_for_quiz_type_uses_right_templates_and_contexts(self, _):
        ...
        self.assertTemplateUsed(resp, "listening_trainer/for_admin/quiz_type_select.html")
    ...
    def test_admin_access_for_eiken_quiz_type_uses_right_templates_and_contexts(self, _):
        ...
        self.assertTemplateUsed(resp, "listening_trainer/for_admin/eiken_quiz_type_select.html")
        self.assertEqual(resp.context["student"], self.class1_1_active_student)
        self.assertEqual(resp.context["classroom_id"], str(self.class1_1.id))
    ...
    def test_admin_quiz_type_select_returns_no_vocab_when_no_progress(self):
        ...
        self.assertTemplateUsed(resp, "listening_trainer/for_admin/no_vocab_available.html")
    ...
    def test_student_quiz_type_select_rejects_inactive_logged_in_user_by_force_login(self):
        """
        非アクティブ生徒によるログインは弾かれる
        """
        self.client.force_login(self.class1_1_inactive_student)

        for url in [self.url_to_quiz_type_select, self.url_to_eiken_quiz_type_select]:
            with self.subTest(url=url):
                response = self.client.get(url)
                self.assertEqual(response.status_code, 302)  # not authenticatedから
                self.assertRedirects(response, reverse("accounts_auth:login"))
...
class StudentBothQuizTypeSelectTest(TestCase):
    ...
    @patch("listening_trainer.views.student_views.has_vocab_progress", return_value=True)
    ...
    def test_student_access_for_quiz_type_uses_right_templates_and_contexts(self, _):
        ...
        self.assertTemplateUsed(resp, "listening_trainer/for_student/quiz_type_select.html")
        self.assertEqual(resp.context["student"], self.class1_1_active_student)
    ...
    def test_student_access_for_eiken_quiz_type_uses_right_templates_and_contexts(self, _):
        ...
        self.assertTemplateUsed(resp, "listening_trainer/for_student/eiken_quiz_type_select.html")
        self.assertEqual(resp.context["student"], self.class1_1_active_student)

    @patch("listening_trainer.views.student_views.has_vocab_progress", return_value=True)
    @patch("listening_trainer.views.student_views.select_passages_for_student")
    ...
    def test_student_quiz_type_select_returns_no_vocab_when_no_progress(self):
        ...
        resp = self.client.get(self.url_to_quiz_type_select)
        self.assertEqual(resp.status_code, 200)
        self.assertTemplateUsed(resp, "listening_trainer/for_student/no_vocab_available.html")
    ...
    def test_student_quiz_type_select_rejects_inactive_logged_in_user_by_force_login(self):
        ...
        for url in [self.url_to_quiz_type_select, self.url_to_eiken_quiz_type_select]:
            with self.subTest(url=url):
                response = self.client.get(url)
                self.assertEqual(response.status_code, 302)  # not authenticatedからの遷移
                self.assertRedirects(response, reverse("accounts_auth:login"))

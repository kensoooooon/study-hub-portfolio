"""
Issue #162 回帰テスト: read_trainer/models.py の各 *QuerySet.visible_to() から
...
from read_trainer.models import (
    ReadingAnswer,
    ReadingPassage,
    ReadingQuestion,
    StudentReadingPassageProgress,
)


class ReadTrainerVisibleToSuperuserRegressionTests(TestCase):
    @classmethod
    ...
    def _make_answer(student, question):
        return ReadingAnswer.objects.create(
            student=student,
            question=question,
            selected_option="A",
            is_correct=True,
    ...
    def setUp(self):  # 毎回実行されるsuperuserへの一時的変更
        # ロールオブジェクトのキャッシュを避けるため取り直し、is_superuser を立てる
    ...
    def test_passage_visible_to_is_not_widened_by_is_superuser(self):
        """
        ReadingPassageQuerySet.visible_to: is_superuser でも他組織の長文は見えない。
        """
        qs = ReadingPassage.objects.visible_to(self.su_admin)
    ...
    def test_question_visible_to_is_not_widened_by_is_superuser(self):
        """
        ReadingQuestionQuerySet.visible_to: is_superuser でも他組織の設問は見えない。
        """
        qs = ReadingQuestion.objects.visible_to(self.su_admin)
    ...
    def test_answer_visible_to_is_not_widened_by_is_superuser(self):
        """
        ReadingAnswerQuerySet.visible_to: is_superuser でも他組織の解答は見えない。
        """
        qs = ReadingAnswer.objects.visible_to(self.su_admin)
    ...
    def test_progress_visible_to_is_not_widened_by_is_superuser(self):
        """
        StudentReadingPassageProgressQuerySet.visible_to: is_superuser でも
        他組織の進捗は見えない。
        """
        qs = StudentReadingPassageProgress.objects.visible_to(self.su_admin)

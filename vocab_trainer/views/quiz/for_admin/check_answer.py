...
def check_answer_with_admin(request):
    ...
    try:
        ...
        logctx["student_id_from_input"] = student_id
        user = request.user
        logctx["user_id"] = user.id
        student = get_accessible_student_by_uuid_or_404(user, student_id)
        logctx["student_id_from_db"] = str(getattr(student, "id", ""))
        ...
        if quiz_type == "jp_to_en":
            ...
            return error_handling(request, status=400, code="INVALID_QUIZ_TYPE")
        ...
        # ✅ セキュリティ上、relation に本当に含まれているか検証（任意）
        ...
        with transaction.atomic():
            ...
            QuizResult.objects.create(student=student, context=context, is_correct=is_correct)
        ...
        try:
            if not example_sentence:
                step = "generate_example_sentence"
        ...
        step = "next_url"
        if quiz_mode == 'all':
            next_url = reverse('vocab_trainer:quiz_all_with_admin')
        elif quiz_mode == 'chapter':
            next_url = reverse('vocab_trainer:quiz_chapter_with_admin')
        elif quiz_mode == 'review':
            next_url = reverse('vocab_trainer:quiz_review_with_admin')
        else:
            next_url = reverse('vocab_trainer:quiz_type_select_with_admin')

...
すでに存在している教科書、年度、品詞に対してのみ登録作業を実施

- data全体からの読み取り
python manage.py update_vocab_data

- 特定ファイルからの読み取り
python manage.py update_vocab_data --file "data/vocab_trainer/New Horizon/2025年度/3年/名詞.tsv"

- 特定ディレクトリからの読み取り
eg. python manage.py update_vocab_data --dir "data/vocab_trainer/New Horizon/2025年度/3年"
"""
...
class Command(BaseCommand):
    help = "指定TSVまたはディレクトリまたは全体から既存語彙データをチャプター順・品詞ごとに更新する"

    def add_arguments(self, parser):
        parser.add_argument(
            '--file',
            type=str,
            help='更新対象のTSVファイルパス（相対または絶対パス）'
        )
        parser.add_argument(
            '--dir',
            type=str,
            help='更新対象のディレクトリパス（相対または絶対パス）'
        )
    ...
    def handle(self, *args, **options):
        ...
        if file_arg:
            ...
            if not file_path.exists():
                ...
                return
            self.stdout.write(self.style.SUCCESS(f"単一ファイル更新: {file_path}"))
            self.process_update(file_path)
            ...
            self.stdout.write(self.style.SUCCESS(f"ディレクトリ一括更新: {total_files} ファイル in {dir_path}"))

            for idx, file_path in enumerate(file_list, start=1):
                self.stdout.write(self.style.SUCCESS(f"更新中 {idx}/{total_files}: {file_path}"))
                self.process_update(file_path)
        ...
        self.stdout.write(self.style.SUCCESS(f"更新完了: {elapsed_time:.2f} 秒"))
    ...
    def process_update(self, file_path):
        ...
        if not textbook:
            self.stdout.write(self.style.WARNING(
                f"スキップ: {textbook_name} 中{grade}年（{publication_year}年度）の教科書は存在しないため、更新対象外"
            ))
            return
        ...
        part_of_speech = PartOfSpeech.objects.filter(name=pos_en).first()
        if not part_of_speech:
            self.stdout.write(self.style.WARNING(f"スキップ: 品詞 '{pos_en}' は未登録のため、更新対象外"))
            return

        existing_chapters = Chapter.objects.filter(textbook=textbook)
        exists_for_textbook = WordMeaningContext.objects.filter(
            chapter__in=existing_chapters,
            part_of_speeches__part_of_speech=part_of_speech
        ).exists()

        if not exists_for_textbook:
            self.stdout.write(self.style.WARNING(
                f"スキップ: {textbook_name} 中{grade}年（{publication_year}年度, 品詞: {part_of_speech_jp}）のデータは存在しないため、更新対象外"
            ))
            return
        ...
        with open(file_path, encoding="utf-8-sig") as f:
            ...
            for idx, row in enumerate(rows, start=1):
                ...
                self.stdout.write(f"  {idx}/{len(rows)} 単語更新中: {word}")

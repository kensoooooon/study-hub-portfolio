...
class ProblemSelectViewTest(TestCase):
    ...
    def test_org_admin_can_access_student(self):
        ...
        url = reverse("math_trainer:junior_high2:problem_select")
    ...
    def test_teacher_cannot_access_student_without_assignment(self):
        """講師は担当していない生徒にアクセス不可（同教室でも担当外）"""
        self.login_as_teacher()
        url = reverse("math_trainer:junior_high2:problem_select")
        resp = self.client.get(url, data={"student_id": self.class1_1_active_student_without_teacher.id})
        self.assertEqual(resp.status_code, 403)
    ...
    def test_student_cannot_access_any_other_student(self):
        ...
        for student in [
            ...
            with self.subTest(student=student.username):
                resp = self.client.get(url, data={"student_id": student.id})
                self.assertEqual(resp.status_code, 403)


class JuniorHigh2DisplayDispatcherViewTest(TestCase):
    """
    中学2年生の display 割り当てビュー用テスト

    Do:
        境界
        カテゴリー指定のありorなし
        係数・問題タイプ・解のタイプの指定ありorなし
        正常にアクセスした場合は解答画面へ遷移
    """
    @classmethod
    def setUpTestData(cls):
    ...
    def access_to_junior_high2_display_dispatcher(self, parms):
        url = reverse("math_trainer:junior_high2:dispatcher_display")
    ...
    def test_anonymous_user_redirect_to_login(self):
        ...
        data = {
            ...
            "used_coefficient": ["integer", "decimal"],
            "equation_type": ["ax+by=c"],
            "answer_type": ["only_integer"],
        }
        resp = self.access_to_junior_high2_display_dispatcher(data)
    ...
    def test_org_admin_can_access(self):
        ...
        expected_url = build_url(
            reverse("math_trainer:junior_high2:simultaneous_equations_display"),
    ...
    def test_invalid_category_redirect_to_problem_select(self):
        ...
        self.assertEqual(resp.status_code, 302)
        expected = build_url(
            reverse("math_trainer:junior_high2:problem_select"),
            self.class1_1_active_student.id,
            self.class1_1.id,
        )
        self.assertRedirects(resp, expected)

    def test_missing_used_coefficient_redirect(self):
        """使用する係数が指定されていない場合はリダイレクト"""
        self.login_as_org_admin()
        data = {
            "student_id": self.class1_1_active_student.id,
            "classroom_id": self.class1_1.id,
    ...
    def test_missing_equation_type_redirect(self):
    ...
    def test_missing_answer_type_redirect(self):
        """解のタイプが指定されていない場合はリダイレクト"""
        self.login_as_org_admin()
        data = {
            "student_id": self.class1_1_active_student.id,
            "classroom_id": self.class1_1.id,
...
class JuniorHigh2PrintDispatcherViewTest(TestCase):
    """
    中学2年生の print 割り当てビュー用テスト
    ...
    def access_to_junior_high2_print_dispatcher(self, parms):
        url = reverse("math_trainer:junior_high2:dispatcher_print")
        resp = self.client.post(url, data=parms)
        return resp
    ...
    def test_anonymous_user_redirect_to_login(self):
        ...
        resp = self.access_to_junior_high2_print_dispatcher(data)
        self.assertEqual(resp.status_code, 302)
        self.assertRedirects(resp, reverse("accounts_auth:login"))

    def test_org_admin_can_access(self):
        """組織管理者は自身の管理組織に所属した生徒相手に正しい情報があればアクセス可能で、プリント画面へ遷移"""
        self.login_as_org_admin()
        data = {
            "student_id": self.class1_1_active_student.id,
            "classroom_id": self.class1_1.id,
        ...
        resp = self.access_to_junior_high2_print_dispatcher(data)
        expected_url = build_url(
            reverse("math_trainer:junior_high2:simultaneous_equations_print"),
            self.class1_1_active_student.id,
            self.class1_1.id,
        )
        self.assertRedirects(resp, expected_url)
    ...
    def test_classroom_admin_can_access(self):
        """教室管理者は自身の管理している教室に所属した生徒相手に正しい情報があればアクセス可能で、プリント画面へ遷移"""
        self.login_as_classroom_admin()
        data = {
            "student_id": self.class1_1_active_student.id,
            "classroom_id": self.class1_1.id,
    ...
    def test_teacher_can_access(self):
        """講師は担当している生徒相手に正しい情報があればアクセス可能で、プリント画面へ遷移"""
        self.login_as_teacher()
        data = {
            "student_id": self.class1_1_active_student.id,
            "classroom_id": self.class1_1.id,
    ...
    def test_student_can_access(self):
        """生徒は自分自身に正しい情報があればアクセス可能で、プリント画面へ遷移"""
        self.login_as_student()
        data = {
            "student_id": self.class1_1_active_student.id,
            "classroom_id": self.class1_1.id,
    ...
    def test_not_allowed_paper_number_redirect_to_problem_select(self):
        """ALLOWED_PAPER_NUMBERSに含まれない枚数は問題選択へリダイレクト"""
        self.login_as_org_admin()
        data = {
            "student_id": self.class1_1_active_student.id,
            "classroom_id": self.class1_1.id,

...
class Grade2DisplayDispatcherViewTest(TestCase):
    """
    小学2年生の割り当てビュー用テスト
    ...
    def access_to_grade2_dispatcher(self, parms):
        url = reverse("math_trainer:elementary2:dispatcher_display")
    ...
    def test_anonymous_user_redirect_to_login(self):
        ...
        data = {
            ...
            "problem_type": ["read_time", "time_delta_without_am_pm_with_picture"],
            "width_of_time": ["less_than_one_hour", "greater_than_or_equal_to_one_hour"],
        }
        resp = self.access_to_grade2_dispatcher(data)
    ...
    def test_org_admin_can_access(self):
        ...
        expected_url = build_url(
            reverse("math_trainer:elementary2:clock_display"),
    ...
    def test_invalid_category_redirect_to_problem_select(self):
        ...
        resp = self.access_to_grade2_dispatcher(data)
        self.assertEqual(resp.status_code, 302)
        expected = build_url(reverse("math_trainer:elementary2:problem_select"), self.class1_1_active_student.id, self.class1_1.id)
    ...
    def test_missing_width_of_time_redirect(self):
        """
        時間幅が指定されていない場合はリダイレクト
        """
        self.login_as_org_admin()
        data = {
            "student_id": self.class1_1_active_student.id,
...
class Grade2PrintDispatcherViewTest(TestCase):
    """
    小学2年生の割り当てビュー用テスト
    ...
    def access_to_grade2_dispatcher(self, parms):
        url = reverse("math_trainer:elementary2:dispatcher_print")
        resp = self.client.post(url, data=parms)
        return resp
    ...
    def test_org_admin_can_access(self):
        ...
        resp = self.access_to_grade2_dispatcher(data)
        expected_url = build_url(
            reverse("math_trainer:elementary2:clock_print"),
            self.class1_1_active_student.id,
            self.class1_1.id,
        )
        self.assertRedirects(resp, expected_url)
